...
import random
import torch
from soft_colour_processor import SoftColourLogitsProcessor, SoftColourDetector
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="The amount/bias to add to each of the redlist token logits before each token sampling step.",
    )
    parser.add_argument(
        "--normalizers",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="Whether to turn on soft watermarking",
    )
    parser.add_argument(
        "--load_fp16",
        type=str2bool,
...
def generate(prompt, args, flag, model=None, device=None, tokenizer=None):
    ...
    if flag:
        watermark_processor = SoftColourLogitsProcessor(vocab=list(tokenizer.get_vocab().values()),
                                        gamma=args.gamma,
                                        delta=args.delta,
                                        alpha=args.alpha,
...
def format_names(s):
    ...
    s=s.replace("green_green", "Green Green Sequence Counted")
    s=s.replace("green_red", "Green Red Sequence Counted")
    s=s.replace("red_green", "Red Green Sequence Counted")
    s=s.replace("red_red", "Red Red Sequence Counted")
    s=s.replace("ggf", "Green Green Fraction")
    s=s.replace("grf", "Green Red Fraction")
    s=s.replace("rgf", "Red Green Fraction")
    s=s.replace("rrf", "Red Red Fraction")
    return s
...
def list_format_scores(score_dict, detection_threshold):
    ...
    for k,v in score_dict.items():
        if k=='green_fraction' or k =='ggf' or k =='grf' or k == 'rgf' or k == 'rrf': 
...
def detect(input_text, args, flag,threshold, device=None, tokenizer=None):
    """Instantiate the WatermarkDetection object and call detect on
        the input text returning the scores and outcome of the test"""
  
    if flag:
        watermark_detector = SoftColourDetector(vocab=list(tokenizer.get_vocab().values()),
                                            gamma=args.gamma,
                                            seeding_scheme=args.seeding_scheme,
                                            device=device,
                                            tokenizer=tokenizer,
...
def main(args): 
    ...
    if not args.skip_model_load:
        ...
        for i in range(0,1,1):
            ...
            for example in dataset['train']:
                ...
                with_watermark_detection_result = detect(decoded_output_with_watermark, 
                                                        ...
                                                        device=device, 
                                                        tokenizer=tokenizer)
                print("Z_SCORE:", with_watermark_detection_result)
                print(type(with_watermark_detection_result))
                ...
                for i in without_watermark_detection_result[0]:
                    if i[0] == 'Prediction':
                        if i[1] == 'Human/Unwatermarked':
                            tn += 1
                        else:
                            fp +=1
                ...
                for i in with_watermark_detection_result[0]:
                    if i[0] == 'Prediction':
                        if i[1] == 'Watermarked':
                            tp += 1
                        else:
                ...
                fplis.append([fpr, tpr])
                print("NIGGER", fplis)
            
                if count >= 10:
                    break

            # Assuming fplis is a list of lists containing FPR and TPR values for each threshold

            if flag:
                for coordinates in fplis:
                    if (type(coordinates[0]) == float or type(coordinates[0]) == int )and (type(coordinates[1]) == float or type(coordinates[1]) == int):
                        fpr_values.append(coordinates[0])
                        tpr_values.append(coordinates[1])
            else:
                for coordinates in fplis:
                    if (type(coordinates[0]) == float or type(coordinates[0]) == int )and (type(coordinates[1]) == float or type(coordinates[1]) == int):
                        fp_values.append(coordinates[0])
                        tp_values.append(coordinates[1])
            print("BIGERR", fpr_values, tpr_values)
        ...
        plt.plot(fpr_values, tpr_values, linestyle='-', color='blue', label="modified")
        #plt.scatter(fpr_values, tpr_values,label="modified")
      #  plt.scatter(fp_values, tp_values, label="original", color='red')
